...
class qrcode_history extends my_controller{
	...
	public function update_mrp(){
		...
		$data = $this->db_operations->get_record('barcode_readymade_master', ['brmm_id' => $id]);
		if(empty($data)) return ['msg' => 'Barcode not found.'];
		if($data[0]['brmm_delete_status'] == 1) return ['msg' => 'Barcode is deleted.'];

		$this->db->trans_begin();
		if($this->db_operations->data_update('purchase_readymade_trans', ['prmt_mrp' => $mrp], 'prmt_id', $data[0]['brmm_prmt_id']) < 1){
			$this->db->trans_rollback();
			return ['msg' => 'Purchase data not updated'];
		}
		if($this->db_operations->data_update('barcode_readymade_master', ['brmm_mrp' => $mrp], 'brmm_id', $id) < 1){
			$this->db->trans_rollback();
