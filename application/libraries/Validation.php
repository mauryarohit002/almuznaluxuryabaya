<?php 
    ...
    class validation {
        ...
        public function app_type(){
            $result = $this->isKeyExists($this->app_type);
            if(!isset($result['status']) || (isset($result['status']) && $result['status'] === FALSE)) return $result;
            $app_type = $this->headers[$result['data']];
            $data = $this->CI->db->get_where('app_type_master', ['app_type_id' => $app_type])->result_array();
            if(empty($data)) return ['message' => 'App Type not found.'];
            if($data[0]['app_type_status'] == 0) return ['message' => 'App Type has been deactivated.', 'code' => REST_Controller::HTTP_UNAUTHORIZED];
            return ['status' => TRUE, 'data' => $app_type];
        }
