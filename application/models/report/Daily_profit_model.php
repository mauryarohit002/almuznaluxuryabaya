...
class daily_profit_model extends my_model {
    ...
    public function get_record(){
        ...
        if(isset($_REQUEST['_customer']) && !empty($_REQUEST['_customer'])){
            $subsql .= " AND customer.customer_name = '".$_REQUEST['_customer']."'";
        ...
        if(isset($_GET['_entry_date_from']) && !empty($_GET['_entry_date_from'])){
            $subsql .= " AND om.om_em_entry_date >= '".date('Y-m-d',strtotime($_GET['_entry_date_from']))."'";
            $record['filter']['_entry_date_from']= $_REQUEST['_entry_date_from'];
        }else{
            $today = date('d-m-Y');
            $subsql .= " AND om.om_em_entry_date >= '".date('Y-m-d')."'";
            $record['filter']['_entry_date_from']= $today;
        }
        /* =========================================================
           DATE TO
        ========================================================= */
        if(isset($_GET['_entry_date_to']) && !empty($_GET['_entry_date_to'])){
            $subsql .= " AND om.om_em_entry_date <= '".date('Y-m-d',strtotime($_GET['_entry_date_to']))."'";
            $record['filter']['_entry_date_to']= $_REQUEST['_entry_date_to'];
        }else{
            $today = date('d-m-Y');
            $subsql .= " AND om.om_em_entry_date <= '".date('Y-m-d')."'";
            $record['filter']['_entry_date_to'] = $today;
        }
        ...
        SELECT
		    ...
		    GROUP_CONCAT(DISTINCT UPPER(sku.sku_name)SEPARATOR ', ') as sku_name,
		    GROUP_CONCAT(DISTINCT UPPER(apparel.apparel_name)SEPARATOR ', ') as apparel_name,
		    UPPER(branch.branch_name) as branch_name,
		    IFNULL(UPPER(ot.ot_trans_type),'READYMADE') as order_type,
		    COUNT(ot.ot_id) as total_items,
		    SUM(IFNULL(ot.ot_qty,0)) as total_qty,
		    SUM(IFNULL(ot.ot_taxable_amt,0)) as selling_price,
		    SUM((IFNULL(ot.ot_sku_cp,0))) as cost_price,
		    (SUM(IFNULL(ot.ot_taxable_amt,0)) - SUM((IFNULL(ot.ot_sku_cp,0)))) as profit_amt,
		    ROUND(((SUM(IFNULL(ot.ot_taxable_amt,0))-SUM((IFNULL(ot.ot_sku_cp,0))))/NULLIF(SUM(IFNULL(ot.ot_taxable_amt,0)),0)) * 100,2) as profit_percent,
	        om.om_total_amt,
	        om.om_advance_amt,
	        om.om_allocated_amt,
        ...
        AND om.om_branch_id !=".SAMPLING_BRANCH."
        AND (om.om_total_amt-om.om_return_amt)>0
        $subsql
        GROUP BY om.om_id
        ORDER BY om.om_em_entry_date DESC
        ";
       
        $record['profit_data'] = $this->db->query($query)->result_array();
        $record['summary'] = [
            'total_sales'      => 0,
            'total_cost'       => 0,
            'gross_profit'     => 0,
        ...
        if(!empty($payment_modes)){
            foreach($payment_modes as $key => $value){
                $payment_query = "
                    SELECT SUM(IFNULL(opmt.opmt_amt,0)) as amount
                    FROM order_payment_mode_trans opmt
                    INNER JOIN order_master om ON(om.om_id = opmt.opmt_om_id)
                    INNER JOIN customer_master customer ON(customer.customer_id= om.om_customer_id)
                    INNER JOIN branch_master branch ON(branch.branch_id= om.om_branch_id)
                    WHERE om.om_delete_status = 0
                    ...
                    AND (om.om_total_amt-om.om_return_amt)>0
                    AND om.om_branch_id !=".SAMPLING_BRANCH."
                    AND opmt.opmt_payment_mode_id= '".$value['payment_mode_id']."'
                    $subsql
                ";
                $payment_result
                    = $this->db
        ...
        ========================================================= */
        $general_query = "
            SELECT SUM(IFNULL(payment_general_amt,0)) as total
        ...
        ========================================================= */
        $payment_query = "
            SELECT SUM(IFNULL(pprt.pprt_adjust_amt,0)) as total
            FROM payment_purchase_readymade_trans pprt
            INNER JOIN payment_master payment ON(payment.payment_id= pprt.pprt_payment_id)
